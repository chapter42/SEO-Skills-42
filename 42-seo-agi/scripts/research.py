...
Options:
    ...
    --region=CODE        Country shortcut: nl, de, be, fr, uk, us (sets location + language)
    --location=CODE      DataForSEO location code (overrides --region)
    --language=CODE      Language code (overrides --region)
...
# DataForSEO location codes: https://api.dataforseo.com/v3/appendix/dataforseo_labs/locations_and_languages
REGION_MAP = {
    "nl": {"location": 2528, "language": "nl"},
    "be": {"location": 2056, "language": "nl"},  # Belgium, Dutch
    "be-fr": {"location": 2056, "language": "fr"},  # Belgium, French
    "de": {"location": 2276, "language": "de"},
    "fr": {"location": 2250, "language": "fr"},
    "uk": {"location": 2826, "language": "en"},
    "us": {"location": 2840, "language": "en"},
    "es": {"location": 2724, "language": "es"},
    "it": {"location": 2380, "language": "it"},
    "pt": {"location": 2620, "language": "pt"},
    "br": {"location": 2076, "language": "pt"},
    "au": {"location": 2036, "language": "en"},
    "ca": {"location": 2124, "language": "en"},
    "at": {"location": 2040, "language": "de"},  # Austria
    "ch": {"location": 2756, "language": "de"},  # Switzerland
    "se": {"location": 2752, "language": "sv"},  # Sweden
    "dk": {"location": 2208, "language": "da"},  # Denmark
    "no": {"location": 2578, "language": "no"},  # Norway
    "pl": {"location": 2616, "language": "pl"},  # Poland
    "jp": {"location": 2392, "language": "ja"},  # Japan
}


def _detect_region_from_keyword(keyword: str) -> str | None:
    """Auto-detect region from common Dutch/German/French words in keyword."""
    kw = keyword.lower()
    # Dutch indicators
    nl_words = [
        " van ", " voor ", " het ", " een ", " zijn ", " worden ", " naar ",
        " wat ", " hoe ", " waar ", " beste ", " boeken ", " inhuren ",
        " spreker", " specialist", " advies", " opleiding", " kosten",
    ]
    if any(w in f" {kw} " or kw.endswith(w.strip()) for w in nl_words):
        return "nl"
    # German indicators
    de_words = [" für ", " und ", " der ", " die ", " das ", " wie ", " berater"]
    if any(w in f" {kw} " or kw.endswith(w.strip()) for w in de_words):
        return "de"
    # French indicators
    fr_words = [" pour ", " les ", " des ", " dans ", " avec ", " comment "]
    if any(w in f" {kw} " for w in fr_words):
        return "fr"
    return None
...
def parse_args():
    ...
    parser.add_argument(
        "--region",
        default=None,
        help="Country shortcut: nl, de, be, fr, uk, us, es, it, br, au",
    )
    parser.add_argument(
        "--location", type=int, default=None, help="Location code (overrides --region)"
    )
    parser.add_argument(
        "--language", default=None, help="Language code (overrides --region)"
    )
    parser.add_argument(
        "--output",
...
def run_research(args) -> dict:
    ...
    # Region resolution: --location/--language override --region override auto-detect override config default
    region_key = args.region
    if not region_key and not args.location and not args.language:
        region_key = _detect_region_from_keyword(args.keyword)
        if region_key:
            print(f"Auto-detected region: {region_key} (from keyword)", file=sys.stderr)

    if region_key and region_key.lower() in REGION_MAP:
        region = REGION_MAP[region_key.lower()]
        location = args.location or region["location"]
        language = args.language or region["language"]
        print(f"Region: {region_key} → location={location}, language={language}", file=sys.stderr)
    elif region_key:
        print(f"Unknown region '{region_key}'. Available: {', '.join(sorted(REGION_MAP))}", file=sys.stderr)
